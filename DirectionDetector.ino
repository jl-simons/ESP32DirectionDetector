/**
   ESP32-CAM Directional Motion Detection Modified for Counting People
...
int objectCount = 0;
int nextObjectID = 1;
int peopleCount = 0; // Counter for people entering/exiting
...
std::vector<std::vector<std::pair<int, int>>> getBlobs(std::vector<std::pair<int, int>> &movingBlocks);
...
bool motion_detect() {
    ...
    for (auto &centroid : centroids) {
        ...
        for (int i = 0; i < objectCount; i++) {
            float distance = sqrt(pow(objects[i].x - centroid.first, 2) + pow(objects[i].y - centroid.second, 2));
                ...
                for (int dx = -1; dx <= 1; dx++) {
                    for (int dy = -1; dy <= 1; dy++) {
                        ...
                        if (nx >= 0 && nx < W && ny >= 0 && ny < H && !visited[ny][nx]) {
                            if (std::find(movingBlocks.begin(), movingBlocks.end(), std::make_pair(nx, ny)) != movingBlocks.end()) {
